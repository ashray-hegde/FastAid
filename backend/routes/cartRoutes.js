...
router.post("/checkout", verifyToken, async (req, res) => {
  try {
    const { paymentMethod = "cod", couponCode, tip = 0, location, coordinates, locationDetails = {} } = req.body;
    const cart = await Cart.findOne({ userId: req.user.id });
    ...
    for (const item of cart.items) {
      ...
      const booking = await Booking.create({
        ...
        paymentMethod,
        transactionId: paymentMethod !== "cod" ? `TX-${Date.now()}-${Math.random().toString(36).substr(2, 6)}` : null,
        tip: Math.round(servicePrice * tipShare * 100) / 100,
      ...
      if (io && providerId) {
        io.emit("booking-created", { booking, providerId });
      }
